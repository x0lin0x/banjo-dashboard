...
@router.post("/trades")
def sync_trades(symbol: str = Query(default="BTCUSDT", min_length=3, max_length=20), limit: int = Query(default=100, ge=1, le=1000), db: Session = Depends(get_db)) -> dict:
...
@router.post("/all")
def sync_all(symbol: str = Query(default="BTCUSDT", min_length=3, max_length=20), limit: int = Query(default=100, ge=1, le=1000), db: Session = Depends(get_db)) -> dict:
    inserted = binance_sync_service.sync_trades(db=db, symbol=symbol.upper(), limit=limit)
    updated = binance_sync_service.sync_positions(db=db)
